...
public class LoginController {
    ...
    public void initialize() {
        cine = Cine.getInstancia(); //instancia el sistema cine
    }

    @FXML
    public void login() {

        String email = emailField.getText().trim();//quita los espacios al inicio y final
    ...
    public void registrar() {
        ...
        // validar email
        if (email.length() < 10 || !email.contains("@") || !email.endsWith(".com") && !email.endsWith(".ar")) {
            mostrarAlerta(Alert.AlertType.ERROR, "ERROR", "Indicar un correo electronico valido.");
            return;
        }
